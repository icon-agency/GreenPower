...
**Where the code lives:** CSS in `scss/utilities/_scroll-animations.scss`; a single `IntersectionObserver` in an inline `<script>` duplicated in `templates/home.html`, `templates/master-template.html` and the root `index.html` (the design system) — in Drupal this becomes ONE shared theme-library behavior. This is THE shared system — never add another observer.
...
- **CSS owns the motion.** The hidden state (`opacity: 0`, `translateY(--animate-distance)`) lives on `.js-animations [data-animate]`, and the `opacity`/`transform` transition lives on the `.is-visible` rule — the reveal path ONLY, so priming the hidden state snaps instead of animating (a transition on the hidden rule animates the prime itself, and above the fold the reveal's reversal completes in 0ms — see LESSONS.md). The script's only job is to add `.is-visible` (which the transition carries to) and to write `--animate-delay` on staggered children.
