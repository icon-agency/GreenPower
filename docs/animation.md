...
**Where the code lives:** CSS in `scss/utilities/_scroll-animations.scss`; a single `IntersectionObserver` in an inline `<script>` duplicated in `templates/home.html`, `templates/master-page.html` and the root `index.html` (the design system) — in Drupal this becomes ONE shared theme-library behavior. This is THE shared system — never add another observer.
